...
// ErrorConverter DeepSeek 协议的错误转换策略。
// DeepSeek 使用 OpenAI 兼容的错误格式。
type ErrorConverter struct {
	logger port.Logger
}

// NewErrorConverter 创建 DeepSeek 错误转换策略实例。
...
func getDefaultMessage(statusCode int) string {
	...
	case http.StatusUnauthorized:
		return "DeepSeek 认证失败"
	case http.StatusForbidden:
		return "DeepSeek 权限不足"
	case http.StatusNotFound:
		return "DeepSeek 资源未找到"
	case http.StatusTooManyRequests:
		return "DeepSeek 请求频率超限"
	case http.StatusInternalServerError:
		return "DeepSeek 内部服务器错误"
	case http.StatusBadGateway:
		return "DeepSeek 网关错误"
	case http.StatusServiceUnavailable:
		return "DeepSeek 服务不可用"
	case http.StatusGatewayTimeout:
		return "DeepSeek 网关超时"
	default:
...
func (c *ErrorConverter) Supports(protocol types.Protocol) bool {
	return protocol == types.ProtocolDeepSeek
}

// Protocol 返回支持的协议类型。
func (c *ErrorConverter) Protocol() types.Protocol {
	return types.ProtocolDeepSeek
}

// Name 返回策略名称。
func (c *ErrorConverter) Name() string {
	return "DeepSeekErrorConverter"
}
