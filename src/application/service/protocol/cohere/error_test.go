...
func TestErrorConverter_Convert(t *testing.T) {
	...
	t.Run("处理原始响应体作为消息", func(t *testing.T) {
		...
		if result.Message != "Cohere API error: invalid request" {
			t.Errorf("期望原始消息, 实际 '%s'", result.Message)
		}

		if result.Provider != "cohere" {
			t.Errorf("期望 provider 'cohere', 实际 '%s'", result.Provider)
		}
	})
	...
	t.Run("处理空响应体使用默认消息", func(t *testing.T) {
		...
		if result.Message != "Cohere 请求参数无效" {
...
func TestErrorConverter_Protocol(t *testing.T) {
	...
	if converter.Protocol() != types.ProtocolCohere {
		t.Errorf("期望 Protocol Cohere, 实际 %s", converter.Protocol())
	}
}
...
func TestErrorConverter_Name(t *testing.T) {
	...
	if converter.Name() != "CohereErrorConverter" {
		t.Errorf("期望名称 'CohereErrorConverter', 实际 '%s'", converter.Name())
	}
}
...
// TestErrorConverter_CohereOnlyProtocol 测试 Cohere 协议的独占性。
func TestErrorConverter_CohereOnlyProtocol(t *testing.T) {
	converter := NewErrorConverter(nil)

	// Cohere 应该是唯一支持 Cohere 协议的转换器
	if !converter.Supports(types.ProtocolCohere) {
		t.Error("应支持 Cohere 协议")
	}
	...
	for _, proto := range otherProtocols {
		if converter.Supports(proto) {
			t.Errorf("Cohere 错误转换器不应支持 %s 协议", proto)
		}
	}
}
