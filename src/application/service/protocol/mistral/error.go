...
// ErrorConverter Mistral 协议的错误转换策略。
// Mistral 使用 OpenAI 兼容的错误格式。
type ErrorConverter struct {
	logger port.Logger
}

// NewErrorConverter 创建 Mistral 错误转换策略实例。
...
func getDefaultMessage(statusCode int) string {
	switch statusCode {
	case http.StatusBadRequest:
		return "Mistral 请求参数无效"
	case http.StatusUnauthorized:
		return "Mistral 认证失败"
	case http.StatusForbidden:
		return "Mistral 权限不足"
	case http.StatusNotFound:
		return "Mistral 资源未找到"
	case http.StatusTooManyRequests:
		return "Mistral 请求频率超限"
	case http.StatusInternalServerError:
		return "Mistral 内部服务器错误"
	case http.StatusBadGateway:
		return "Mistral 网关错误"
	case http.StatusServiceUnavailable:
		return "Mistral 服务不可用"
...
func (c *ErrorConverter) Supports(protocol types.Protocol) bool {
	return protocol == types.ProtocolMistral
}

// Protocol 返回支持的协议类型。
func (c *ErrorConverter) Protocol() types.Protocol {
	return types.ProtocolMistral
}

// Name 返回策略名称。
func (c *ErrorConverter) Name() string {
	return "MistralErrorConverter"
}
