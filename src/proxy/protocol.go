...
// Uses tokligence translator internally for robust conversion
...
func (pc *ProtocolConverter) ConvertToAnthropic(openAIBody map[string]interface{}) ([]byte, error) {
	...
	if maxTokens, ok := openAIBody["max_tokens"]; ok {
		meta.InputMaxTokens = maxTokens
		meta.MaxTokensSource = "max_tokens"
		switch v := maxTokens.(type) {
		case float64:
			meta.OutputMaxTokens = int(v)
		case int:
			meta.OutputMaxTokens = v
		}
	} else if maxComp, ok := openAIBody["max_completion_tokens"]; ok {
		meta.InputMaxTokens = maxComp
		meta.MaxTokensSource = "max_completion_tokens"
		switch v := maxComp.(type) {
		case float64:
			meta.OutputMaxTokens = int(v)
		case int:
			meta.OutputMaxTokens = v
		...
		meta.MaxTokensSource = "default"
		meta.OutputMaxTokens = 4096
	}

	if temp, ok := openAIBody["temperature"].(float64); ok {
		meta.InputTemperature = temp
		meta.OutputTemperature = temp
	}

	if topP, ok := openAIBody["top_p"].(float64); ok {
		meta.InputTopP = topP
		meta.OutputTopP = topP
	}

	if stream, ok := openAIBody["stream"].(bool); ok {
		meta.InputStream = stream
		meta.OutputStream = stream
	}
	...
	if tools, ok := openAIBody["tools"].([]interface{}); ok {
		meta.InputTools = len(tools)
		meta.OutputTools = len(tools)
	}

	result, err := pc.adapter.ConvertToAnthropic(openAIBody)
	if err != nil {
		return nil, err
	}
	...
	if err := json.Unmarshal(result, &anthropicBody); err == nil {
		if system, ok := anthropicBody["system"]; ok {
			switch s := system.(type) {
			case string:
				meta.SystemPromptLen = len(s)
			case []interface{}:
				totalLen := 0
				for _, block := range s {
				...
				meta.SystemPromptLen = totalLen
			}
		}
	}

	pc.lastConversion = meta
